...
io.sockets.on('connection', function (socket) {
	...
	var addressa = socket.request.connection.remoteAddress;
	console.log('New connection from ' + addressa);
	...
	uploader.on("complete", function (event) {
		...
		//	var abc = file.pathName;
		//	var pathnamaka = str_replace('upload\\', 'upload/', abc);
		//	console.log("repl" + pathnamaka);
		...
		setTimeout(function () {
			...
			if (isPICfile) {
				...
				//var imagesize = getimagesize('./upload/FaceQ1436941594431.png');
				console.log("path+" + filepath);
				var dimensions = sizeOf(filepath);
				console.log("ima: " + dimensions.width + ", " + dimensions.height);
				var imageSize = [dimensions.width, dimensions.height];
				...
				if (!isGCODEfile) {
					...
					contentt = content.toString();
					opt = options;
				//	options.scale = 0.5*0.28222222222;
	...
	socket.on('imagesize',function(size){
		console.log("set image size"+size);
		opt.scale = size;
		
		options = opt;
		console.log("2"+options);
	//	options.scale = size*0.28222222222;
		//argv.scale = size*0.28222222222;
				//	options.scale = 0.5*0.28222222222;
					//io.sockets.emit("settings", argv);
	...
	socket.on('webcamChangeResolution', function (resolution) {
		...
		io.sockets.emit("mjpg_log", "mjpg_log");
...
//	console.log("B");
    ...
    console.log("GcodetoCliend: "+command);
...
function receiveData(data) {
	...
	//	console.log("data: " + data);
		...
		//console.log("data_array 1: " + data_array[1]);
		//console.log("data_array 2: " + data_array[2]);
		laserPos.set(phpjs.floatval(data_array[1]), phpjs.floatval(data_array[2]));
		console.log("pos: " + laserPos);
     //   console.log("machineRunning "+ machineRunning);
		    //    console.log("machinePause "+ machinePause);
		      //  console.log("copiesDrawing "+ copiesDrawing);
	...
	//	console.log("WS messege " + data);
		...
		for (i = 0; i < split.length; i++) {
			...
			if (response.indexOf("ok") != -1 || response == "start\r" || response.indexOf('<') == 0) {
				...
				// //	console.log("respone 1:" + response);
				...
				// 	receiveData(response);
				// } else {
				// //	console.log("respone 2:" + response);
				// }
				blocked = false;
			}
		}
...
function uploadLine() {
	...
	//	console.log("gcode ws" + gcode);
		// console.log('Sent: '  + gcode + ' Q: ' + gcodeQueue.length)

		// $('#queueCnt').html('Sent: '  + gcode + ' Q: ' + gcodeQueue.length)
		//  lastSent = gcode
	//	sendGcode("ESP8266", gcode + '\n');
		blocked = true;
	}
	//}
}
